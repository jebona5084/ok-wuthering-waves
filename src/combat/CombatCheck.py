...
class CombatCheck(BaseWWTask):
    ...
    def check_f_break(self):
        if not self.can_break and not self._in_liberation and time.time() - self.last_break_check_time > 1:
            ...
            if self.find_one(Labels.f_break_full, threshold=0.9):
                ...
                self.can_break = True
                return True
            if self.find_one('f_break', box=self.box_of_screen(0.2, 0.2, 0.75, 0.8), target_height=720):
    ...
    def check_count_down(self):
        count_down_area = self.box_of_screen_scaled(3840, 2160, 1820, 266, 2100, 340,
                                                    name="check_count_down", hcenter=True)
    ...
    def ensure_levitator(self):
        ...
        if self.in_team()[0]:
            ...
            self.sleep(0.1)
            return False
        levitator = self.wait_feature('wheel_levitator', threshold=0.65,
                                      box=self.box_of_screen(0.27, 0.16, 0.68, 0.76))
        self.sleep(0.1)
        if not levitator:
            self.send_key_up(self.key_config.get('Wheel Key'))
    ...
    def ocr_lv_text(self):
        return self.ocr(box=self.target_area_box,
    ...
    def target_enemy(self, wait=True):
        ...
        Args:
            wait: If True, retry middle-clicks until a target is found or
                  ``target_enemy_time_out`` is reached.  If False, perform a
                  single middle-click and return immediately.

        Returns:
            True  – a target is confirmed on screen.
            False – no target found (timeout or single-click miss).
        """
        if not wait:
            self.middle_click()
