...
class Rover(BaseChar):
    ...
    def do_perform(self):
        ...
        if self.ring_index == Elements.HAVOC:
            ...
            if self.need_fast_perform():
                self.logger.info('rover: form-dispatch ring=WIND routine=wind-fast')
    ...
    def init_wind(self):
        ...
        self.decide_teammates()
        # name-based scan catches custom-loaded subclasses; has_char covers
        # the built-in classes (and tasks that don't expose a chars list)
        chars = getattr(self.task, 'chars', None) or []
        has_cartethyia = (self.char_cartethyia is not None
                          or bool(self.task.has_char(Cartethyia)))
        has_phoebe = (any(c is not None and type(c).__name__ == 'Phoebe'
                          for c in chars)
                      or bool(self.task.has_char(Phoebe)))
        self.use_skyfall_severance = has_cartethyia and has_phoebe
