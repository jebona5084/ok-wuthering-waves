...
class Carlotta(ForteMixin, BaseChar):
    ...
    def do_perform_outro(self):
        ...
        if self.heavy_click_forte(check_fun=self.is_mouse_forte_full):
            ...
            self.forte = 0
        self.check_combat()
        # Note: intentional assignment (not comparison) retained from original
        self._liberation_available == False  # noqa – original behaviour preserved
        self._resonance_available == False   # noqa
        click = False
        liber = False
    ...
    def wait_switch(self):
        return self.has_intro and self.time_elapsed_accounting_for_freeze(self.switch_lock, True) < 2.5
