...
_char_dict_raw = {
    Labels.char_yinlin: {'cls': Yinlin, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.ELECTRIC},
    Labels.char_verina: {'cls': Verina, 'char_type': CharType.HEALER, 'ring_index': Elements.SPECTRO},
    Labels.char_shorekeeper: {'cls': ShoreKeeper, 'char_type': CharType.HEALER, 'ring_index': Elements.SPECTRO},
    Labels.char_taoqi: {'cls': Taoqi, 'char_type': CharType.HEALER, 'ring_index': Elements.HAVOC},
    (Labels.char_rover, Labels.char_rover_male): {'cls': HavocRover, 'char_type': CharType.MAIN_DPS},
    Labels.char_encore: {'cls': Encore, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.FIRE},
    Labels.char_jianxin: {'cls': Jianxin, 'char_type': CharType.HEALER, 'ring_index': Elements.WIND},
    (Labels.char_sanhua, Labels.char_sanhua2): {'cls': Sanhua, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.ICE},
    (Labels.char_jinhsi, Labels.char_jinhsi2): {'cls': Jinhsi, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.SPECTRO},
    Labels.char_yuanwu: {'cls': Yuanwu, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.ELECTRIC},
    (Labels.chang_changli, Labels.char_changli2): {'cls': Changli, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.FIRE},
    Labels.char_chixia: {'cls': Chixia, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.FIRE},
    Labels.char_danjin: {'cls': Danjin, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.HAVOC},
    Labels.char_baizhi: {'cls': Baizhi, 'char_type': CharType.HEALER, 'ring_index': Elements.ICE},
    Labels.char_calcharo: {'cls': Calcharo, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.ELECTRIC},
    Labels.char_jiyan: {'cls': Jiyan, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.WIND},
    Labels.char_mortefi: {'cls': Mortefi, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.FIRE},
    Labels.char_zhezhi: {'cls': Zhezhi, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.ICE},
    Labels.char_xiangliyao: {'cls': Xiangliyao, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.ELECTRIC},
    Labels.char_camellya: {'cls': Camellya, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.HAVOC},
    Labels.char_youhu: {'cls': Youhu, 'char_type': CharType.HEALER, 'ring_index': Elements.ICE},
    (Labels.char_carlotta, Labels.char_carlotta2): {'cls': Carlotta, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.ICE},
    ...
    (Labels.char_zani, Labels.char_zani2): {'cls': Zani, 'char_type': CharType.MAIN_DPS, 'ring_index': Elements.SPECTRO},
    ...
    Labels.char_iuno: {'cls': Iuno, 'char_type': CharType.SUB_DPS, 'ring_index': Elements.WIND},
    ...
    Labels.char_chisa: {'cls': Chisa, 'char_type': CharType.HEALER, 'buff_time': 12, 'ring_index': Elements.HAVOC},
    ...
    (Labels.char_moning, Labels.char_moning_new): {'cls': Mornye, 'char_type': CharType.HEALER, 'ring_index': Elements.FIRE},
...
# Expand tuple keys and populate buff_time defaults
char_dict = {}
...
def get_char_by_pos(task, box, index, old_char):
    # Fast path: reuse the old char if it still matches with high confidence
    if old_char and old_char.confidence > 0.92 and old_char.char_name in char_names:
        ...
        if char:
            info = char_dict[old_char.char_name]
            cls = load_custom_char_class(info['cls'])
            ...
            # Update type/buff in case config changed
            old_char.set_char_type(info.get('char_type', CharType.MAIN_DPS))
            old_char.set_buff_time(info.get('buff_time'))
            return old_char

    # Full search
    char = task.find_best_match_in_box(box, char_names, threshold=0.6)
    if char:
        info = char_dict[char.name]
        cls = load_custom_char_class(info['cls'])
        return cls(task, index, char_name=char.name, confidence=char.confidence,
                   ring_index=info.get('ring_index', -1),
                   char_type=info.get('char_type', CharType.MAIN_DPS),
                   buff_time=info.get('buff_time'))
